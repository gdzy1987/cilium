...
var _ = Describe("K8sPolicyTest", func() {
	...
	Context("Basic Test", func() {
		...
		PIt("TLS policy", func() {
			...
			res = kubectl.ExecPodCmd(
				namespaceForTest, appPods[helpers.App2],
				helpers.CurlFail("--retry 5 -4 %s 'https://artii.herokuapp.com/make?text=cilium&font=univers'", "-v --cacert /cacert.pem"))
			res.ExpectSuccess("Cannot connect from %q to 'https://artii.herokuapp.com/make?text=cilium&font=univers'",
				appPods[helpers.App2])

			res = kubectl.ExecPodCmd(
				namespaceForTest, appPods[helpers.App2],
				helpers.CurlFail("--retry 5 -4 --max-time 15 %s 'https://artii.herokuapp.com:443/fonts_list'", "-v --cacert /cacert.pem"))
